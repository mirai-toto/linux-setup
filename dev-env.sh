...
# Build testuser with the host user's uid/gid so bind-mounted files share the
# same owner on both sides. This is the whole reason we don't chown mounts:
# a bind-mounted file has one numeric owner, so host and container must agree
# on it or one side loses write access (and git reports dubious ownership).
BUILD_ARGS=(--build-arg "USER_UID=$(id -u)" --build-arg "USER_GID=$(id -g)")

if [ "$RM" = true ]; then
  echo "Building Docker image for $DISTRO_DIR..."
  docker build "${BUILD_ARGS[@]}" -t "$IMAGE_NAME" -f "$DOCKERFILE" "$SCRIPT_DIR"
